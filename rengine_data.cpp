#include "rengine_data.h"
#include <my_dir.h>
#include <string.h>
Rengine_data::Rengine_data(void)
{
  data_file = -1;
  number_records = -1;
...
int Rengine_data::create_table(char *path)
{
  DBUG_ENTER("Rengine_data::create_table");
  open_table(path);
  number_records = 0;
...
int Rengine_data::open_table(char *path) {
  DBUG_ENTER("Rengine_data::open_table");
  /*
...
long long Rengine_data::write_row(byte *buf, int length)
{
  long long pos;
  int i;
  int len;
  byte deleted = 0;
  DBUG_ENTER("Rengine_data::write_row");
  /*
...
MySQL flags (set to zero fill and with extra error checking). Returns number of bytes allocated -- <= 0 indicates an error.
*/
i = my_write(data_file, &deleted, sizeof(byte), MYF(0)); memcpy(&len, &length, sizeof(int));
...
long long Rengine_data::update_row(byte *old_rec, byte *new_rec,
{
  long long pos;
  long long cur_pos;
  byte *cmp_rec;
  ...
  byte deleted = 0;
  int i = -1;
  DBUG_ENTER("Rengine_data::update_row");
if (position == 0)
...
if (position == -1) //don't know where it is...scan for it
{
  ...
  */
  ...
  {
pos = read_row(cmp_rec, length, cur_pos); if (memcmp(old_rec, cmp_rec, length) == 0) {
      pos = cur_pos;
      cur_pos = -1;
    }
    else if (pos != -1)
...
i = my_write(data_file, &deleted, sizeof(byte), MYF(0)); memcpy(&len, &length, sizeof(int));
i = my_write(data_file, (byte *)&len, sizeof(int), MYF(0)); pos = i;
i = my_write(data_file, new_rec, length, MYF(0));
}
  DBUG_RETURN(pos);
}
/* delete a record in place */
int Rengine_data::delete_row(byte *old_rec, int length,
                             long long position)
{
  int i = -1;
  long long pos;
  ...
  byte *cmp_rec;
  byte deleted = 1;
  DBUG_ENTER("Rengine_data::delete_row");
  if (position == 0)
    position = header_size; //move past header
  pos = position;
  /*
...
/*
Write the deleted byte set to 1 which marks row as deleted at the current file pointer.
Note: my_write() returns the bytes written or -1 on error
    */
...
int Rengine_data::read_row(byte *buf, int length, long long position) {
  int i;
  int rec_len;
  long long pos;
  byte deleted = 2;
  DBUG_ENTER("Rengine_data::read_row");
  if (position <= 0)
    position = header_size; //move past header
...
/* close file */
int Rengine_data::close_table()
{
  DBUG_ENTER("Rengine_data::close_table");
  if (data_file != -1)
  {
...
int Rengine_data::records()
{
  DBUG_ENTER("Rengine_data::num_records");
  DBUG_RETURN(number_records);
}

/* return number of deleted records */
int Rengine_data::del_records()
{
  DBUG_ENTER("Rengine_data::num_records");
  DBUG_RETURN(number_del_records);
}
/* read header from file */
int Rengine_data::read_header()
{
  int i;
  int len;
  DBUG_ENTER("Rengine_data::read_header");
  if (number_records == -1)
  {
...
int Rengine_data::write_header()
{
int i;
  DBUG_ENTER("Rengine_data::write_header");
  if (number_records != -1)
  {
my_seek(data_file, 0l, MY_SEEK_SET, MYF(0));
...
long long Rengine_data::cur_position()
{
  long long pos;
  DBUG_ENTER("Rengine_data::cur_position");
...
int Rengine_data::trunc_table()
{
  DBUG_ENTER("Rengine_data::trunc_table");
  if (data_file != -1 )
  {
...
/* determine the row size of the data file */ int Rengine_data::row_size(int length)
{
  DBUG_ENTER("Rengine_data::row_size");
